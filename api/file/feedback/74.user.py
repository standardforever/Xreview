...
def upload_file():
    ...
    count = os.getenv('UPLOAD-NUMBER')
    num = int(os.getenv('UPLOAD-NUMBER')) + 1
    dotenv.set_key('.env', 'UPLOAD-NUMBER', str(num))
    ...
    if request.method == 'POST':
        ...
        if (request.form.get("email") and path != ""):
            email = request.form.get("email")
            upload = Upload(email=email, file_path=path, link=link)
